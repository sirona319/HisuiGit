...
public sealed class CarveModule : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float _angle = Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg;//ターゲットへの角度を取得する
        ...
        // X方向の移動量を設定する
        velocity.x = Mathf.Cos(_angle * Mathf.Deg2Rad);

        // Y方向の移動量を設定する
        velocity.y = Mathf.Sin(_angle * Mathf.Deg2Rad);
